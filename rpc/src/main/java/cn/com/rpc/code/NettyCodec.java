...
import cn.com.common.utils.Bytes;
...
public class NettyCodec {
    ...
    class Decoder extends ByteToMessageDecoder {
        ...
        private Object decodeBody(ByteBuf buffer, byte[] header, int length) throws IOException {
            byte type = header[2];
            byte status = header[3];
            byte serialType = header[4];
            long id = Bytes.bytes2long(header, 5);
            Serialization serializer = getSerializer(SERIAL_KEY);
            Header h = new Header();
            h.setId(id);
            h.setEventType(type);
            h.setStatus(status);
            ...
            if (type == FLAG_REQUEST_TYPE) {
                ...
                if (uri.getParameter(Constant.IO_DECODER, Constant.IO_DECODER_DEFAULT_VALUE)) {
                    ...
                    ObjectInput objectInput = serializer.deserialize(in);
    ...
    class Encoder extends MessageToByteEncoder {
        ...
        private void encodeResponse(Response response, ByteBuf byteBuf) throws IOException {
            ...
            Serialization serializer = getSerializer(SERIAL_KEY);
            ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
            ObjectOutput out = serializer.serialize(outputStream);
            out.writeObject(response.getData());
